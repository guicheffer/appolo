<?php
	...
	//title with section
	$title =  $page_name_for_title . " [formulário] -" . " Páginas - " . SITE_NAME . " - " . SYSTEM_NAME ;
...
<script type="text/javascript" src="<?=$util->check_prefix();?>static.guiatech.com.br/library/codemirror/mode/htmlmixed/htmlmixed.js"></script>
...
<script type="text/javascript"><!--
	...
	appolo.configs.pages.page.editing_something = 'form' ;
	appolo.configs.this_is_hidden = <?=$this_is_hidden?> ;
	appolo.configs.page_create_form_send = '<?=PAGE_CREATE_FORM_SEND?>' ;
	appolo.configs.page_create_tmpl_send = '<?=PAGE_CREATE_TMPL_SEND?>' ;
...
<body class="<?php echo $session?>">
	...
	<header>
		<div class="row">
			<h3>Formulário de "<?=$page_name_for_title?>"</h3>
	...
	<div class="row area-warn">
		<?php	if ( isset( $warn ) ) { ?>
			...
			<?php switch ( $warn ) {
				...
				case '9': #danger
					$appolo_gui->render_message( "success", true, "Arquivo de formulário \"<b><i>" . TMPLS_DIRECTORY . $util->get_session_and_clear( 'file_saved' ) . "</i></b>\" criado e salvo com sucesso!.", "animated fadeIn" ) ;
				break;

				case '10': #danger
					$appolo_gui->render_message( "info", true, "Arquivo de formulário \"<b><i>" . TMPLS_DIRECTORY . $util->get_session_and_clear( 'file_saved' ) . "</i></b>\" salvo com sucesso!.", "animated fadeIn" ) ;
				break;
	...
	<div class="content-form-page">
		...
		<form action="<?=( PAGE_CREATE_FORM_SEND )?>" method="post" name="main-form_form" role="form">
			...
			<div class="row code-editing">
				<textarea id="editing" type="form" name="editing"><?php if( file_exists( FORMS_DIR . $config_form ) ){ $util->write_xml_form( $config_form ) ; }else{ ?><group name="group_name" title="Nome do grupo">

	<field name="field_name" type="text" notnull>
		<label>Nome do campo</label>
		<input max="50"/>
		<hint>Dica para o campo</hint>
	</field>
